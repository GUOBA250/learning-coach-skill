...
| 🧮 Algorithms | `Grind algorithms, next Hot100 problem` | Code Caprice style; each block runs due dictation, then next-day warm-up dictation of yesterday's problems, then 3 new medium problems — no blank staring |
...
| Algorithm quota and new-problem 3-step (three-stage block: 30-min due dictation ≤3 problems → next-day warm-up dictation (~6 min each) → new problems; two gates, no new problems until both are cleared; 3 new medium at ~12 min each on standard days, 1-2 hard on hard days, +1 only when exceptionally fresh; block length = 30 + yesterday's count×6 + new count×12 ≈ 84 min — trim new problems when time is short, never skip warm-up; think 3-5 min → read solution → write from memory, same-day dictation verifies understanding/correctness only; overrun rolls forward) | `rules/算法规范.md` drill flow | SKILL.md review cadence, 计划规范 step 6 check 3, 计划模板 today table, sample scenarios 2/10 |
| Next-day warm-up queue (every new problem must be re-written closed-book the next day; auto-enroll, dequeue at next-day dictation; one clean rewrite + correct complexity = welded: no error-book entry, no 1/3/7/15 slot; stuck/wrong/wrong-complexity = enters error book at rung 1; dictation-only protocol, ~6 min fast rewrite, complexity reported aloud; queue is independent — excluded from inventory level and hemostasis stats; not applied to Q&A or projects) | `rules/复习滚动机制.md` section 8 + `rules/算法规范.md` warm-up section | SKILL.md review cadence, 计划规范 step 6 check 3, 复习模板 (opening/process/report), 计划模板 today table/adjustment rules, sample scenarios 2/10 |
...
| Algorithm dictation-only protocol (review and warm-up sessions write code with no framework quiz, two-tier hints signature → key line, wrong-line patch plus same-session rewrite, complexity reported aloud; due reviews use 2/1/0 grading, warm-up uses a binary outcome — one clean rewrite = welded / stuck or wrong = enters error book; reading the solution is normal and never an entry by itself) | `rules/算法规范.md` dictation protocol section | 复习滚动机制 section 1 entry table / section 2 rating table / section 4 algorithm leech / section 8 warm-up queue, sample scenario 10 |
| Four daily-table self-checks (total vs target ±30 min fill/trim, peak-energy ordering, algorithms run due dictation then next-day warm-up then 3 new problems with all three counts written out, project block ≤2 observable points) | `rules/计划规范.md` step 6 | SKILL.md progress section, 计划模板 today/tomorrow tables and adjustment rules, 任务先行规范 section 2, sample scenario 10 |
