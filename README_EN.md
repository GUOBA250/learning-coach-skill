...
- Study plan: given the interview countdown, daily available hours, and your level in interview Q&A / algorithms / projects, it produces a three-phase overview, a today-task table sliced into time blocks, and milestones — check-ins collect four things (blocks done / blockers / per-block load 🔵🟢🟠 / fatigue), and the plan adjusts daily from completion rate (sub-50% days auto-halve, debt never rolls over, a cross-day status block is maintained); after each check-in it also collects actual time spent and, when the gap exceeds 25%, recalibrates upcoming schedules using per-topic coefficients (a single day under 60% of estimate already raises an over-estimate warning; disable-able and manually overridable); blocks over 50 minutes are split 25+5 pomodoro style, saturated-state zeroes are not graded, and each weekly report ends with a "moment it clicked" teaching-method review; written-test and no-project/weak-project edge cases each have dedicated scheduling rules
...
- Algorithms: Code Caprice style — "why this method" first, then "how", emphasizing frameworks, common pitfalls, and complexity; problem signals / approach framework / template skeleton / complexity are 🔴 must-recite (closed-book writing), while boundary details and variants are 🟡 — just understand and adapt; the daily quota floats with state (default 1 new + 1 review, 15 min each for familiar problems, 1-new cap on hard days, overrun problems roll to tomorrow's review slot without cannibalizing other blocks)
- Source reading: a quick learning-style diagnosis picks one of two tracks — listener-track gets a level-tagged function map with bottom-up line-by-line walkthrough; builder-track gets task-first learning (vocab cards → task brief → predict → hands-on verify → mechanism wrap-up → glossary filing), and reading blocks are triaged into four obstacle layers (syntax/API, cross-file data flow, static-to-dynamic, framework vocabulary) with a matching channel; every block still ends with an 🔴 L1 list and review questions
...
| `skill/learning-coach/references/rules/` | Nine rule sets: interview-question four-part structure with patch-based oral correction plus the three-phase reforge, Code Caprice algorithm style (with L1/L2/L3 levels and the flexible daily quota), listener-track line-by-line source reading and builder-track "task-first" learning (learning-style diagnosis, four-layer reading-obstacle triage, personal glossary), interview-countdown study planning (four-field check-in with block load/fatigue, time calibration with the 60% warning, pomodoro/fatigue protection, weekly teaching-method review; incl. written-test and no-project edge cases), mock-interview flow with scoring and debrief, spaced-repetition error book (1/3/7/15-day, with the project-leech channel toolbox, artifact-based retest, inventory level and hemostasis rule), L1/L2/L3 memory priority (shared by Q&A, algorithms, source reading, and mocks), and PlanCoach kickstart — decide "how to teach / plan / examine / retain / what to memorize" |
...
| Four-field check-in and fatigue protection (block load 🔵🟢🟠 linkage, saturated zeroes move to next morning, 25+5 pomodoro, 60% over-estimate warning, weekly teaching-method question) | `rules/计划规范.md` check-in section and section 3 | SKILL.md progress section, 计划模板 (feedback block / status / report / hemostasis day), 复习滚动机制 section 2, sample scenario 5C |
...
| Flexible algorithm quota (default 1 new + 1 review, familiar problems 15 min each, 1-new cap on hard days, overrun rolls forward) | `rules/算法规范.md` drill flow | 计划规范 (pomodoro), 计划模板 feedback-block note, sample scenario 2 |
