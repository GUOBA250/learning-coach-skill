...
- Algorithms: Code Caprice style — "why this method" first, then "how", emphasizing frameworks, common pitfalls, and complexity; problem signals / approach framework / template skeleton / complexity are 🔴 must-recite (closed-book writing), while boundary details and variants are 🟡 — just understand and adapt; **every algorithm block contains ≥2 new medium problems** except hard days, leech/due review takes only 10-15 min and block length follows 15 + 25-30 min per new problem; review sessions run a dictation-only protocol (write code, no framework quiz; hints escalate function signature → key line; wrong lines get a patch and a same-session rewrite); hard days may drop to 1 hard problem, overrun problems roll forward without cannibalizing other blocks
...
| Algorithm quota (≥2 new medium except hard days, leech review 10-15 min never eats new-problem time, length formula 15+25-30 min per new problem, hard days drop to 1 hard, overrun rolls forward) | `rules/算法规范.md` drill flow | SKILL.md review cadence, 计划规范 step 6 check 3, 计划模板 today table, sample scenario 11 |
| Algorithm dictation-only protocol (review sessions write code with no framework quiz, two-tier hints signature → key line, wrong-line patch plus same-session rewrite) | `rules/算法规范.md` dictation protocol section | 复习滚动机制 section 2 rating table / section 4 algorithm leech, sample scenario 11 |
| Four daily-table self-checks (total vs target ±30 min fill/trim, peak-energy ordering, algorithm new problems mandatory, project block ≤2 observable points) | `rules/计划规范.md` step 6 | SKILL.md progress section, 计划模板 today/tomorrow tables and adjustment rules, 任务先行规范 section 2, sample scenario 11 |
