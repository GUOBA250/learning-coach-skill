...
- 背八股：四段式讲解（通俗讲解 + 文字流程图 + 75-80 分核心答案 + 追问），开头先给「🔴 必背清单」，口述之后逐条校正
...
- **背八股** — 开头先给「🔴 必背清单」（2-4 条 L1），再走四段式：通俗讲解 → 文字流程图 → 约 350 字可直接背诵的核心答案 → 面试官追问。之后你合上材料主动回忆，教练只严格卡 L1，逐条校正。
...
2. **每日背八股** — "背八股，从 Vue 章节开始" → 先看 🔴 必背清单 → 复述 → 只卡 L1 的逐条校正 → 下一题。
...
| `skill/learning-coach/references/rules/` | 六份规范：八股四段式、算法卡哥风格、项目逐行讲解、面试倒计时定计划、L1/L2/L3 记忆优先级（八股和项目共用）、PlanCoach 启动学习，决定"怎么讲/怎么排/背什么" |
| `skill/learning-coach/references/templates/` | 四类输出模板：八股（含 🔴 必背清单）、算法、项目（含级别函数地图和 L1 清单）、学习计划的完整输出格式，决定"长什么样" |
