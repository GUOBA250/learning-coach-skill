...
- 刷算法：卡哥风格讲解，先讲"为什么用这个方法"再讲"怎么做"，强调框架、易错点和复杂度；题型信号/思路框架/模板骨架/复杂度是 🔴 必背（闭卷默写），边界细节和变种是 🟡 看懂能改即可；**每个算法块非 hard 日必须 ≥2 道新题（medium 基准）**，钉子/到期复习只占 10-15min、时长按 15+25-30min/新题，复习段走纯默写协议（只默写不问框架，提示按"函数签名→关键行"分档，错行打补丁当天再默），hard 日可减为 1 道 hard、超时顺延不占别的块
...
| `skill/learning-coach/references/rules/` | 九份规范：八股四段式、补丁式口述校正与回炉三段式、算法卡哥风格（含 L1/L2/L3 分级、≥2 新题配额与纯默写协议）、项目听讲型逐行讲解与构造型「任务先行」（学习风格问诊、读码障碍四层分诊、词汇表、每块 1-2 可观测点）、面试倒计时定计划（排程四自检、打卡四件事与负荷次日联动、耗时校准与 60% 预警、番茄钟/疲劳保护、每周教学法一问，含笔试/项目弱边界）、模拟面试流程与评分复盘、错题本 1/3/7/15 天滚动复习（含项目钉子通道工具箱、物证复测与库存水位止血）、L1/L2/L3 记忆优先级（八股、算法、项目、模拟面试共用）、PlanCoach 启动学习，决定"怎么讲/怎么排/怎么考/怎么不忘/背什么" |
...
| 算法配额（非 hard 日 ≥2 道新 medium、钉子复习 10-15min 不占新题、时长公式 15+25-30min/新题、hard 日减为 1 hard、超时顺延） | `rules/算法规范.md` 刷题流程 | SKILL.md 复习节奏、计划规范第六步自检 3、计划模板今日表、示例场景十一 |
| 算法纯默写协议（复习段只默写不问框架、函数签名→关键行两档提示、错行补丁当天再默） | `rules/算法规范.md`「纯默写协议」节 | 复习滚动机制第二节评级表/第四节算法钉子、示例场景十一 |
| 排每日任务四自检（时长对目标 ±30min 必补/必砍、精力曲线排序、算法必有新题、项目块 ≤2 可观测点） | `rules/计划规范.md` 第六步 | SKILL.md 进度记录、计划模板今日表/明日表/调整规则、任务先行规范第二节、示例场景十一 |
