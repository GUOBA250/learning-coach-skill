...
| 🧮 刷算法 | `刷算法，Hot100 下一题` | 卡哥风格讲题；每块先默写到期题、再保温默昨日新题，最后刷 3 道新 medium，不盯题死想 |
...
| 算法配额与新题三步（块内三段：30min 到期默写 ≤3 道 → 昨日新题保温默（×6min）→ 当天新题，两道闸门没默完不加新题；标准日 3 道新 medium、12min/道，hard 日 1-2 hard、状态特好临时 +1 不硬撑；时长 = 30+昨日数×6+新题数×12 ≈ 84min，排不下减新题、不砍保温默；新题想 3-5min → 看题解理解 → 合上默写，当天只验收理解+代码对；超时顺延） | `rules/算法规范.md` 刷题流程 | SKILL.md 复习节奏、计划规范第六步自检 3、计划模板今日表、示例场景二/十 |
| 次日保温默队列（当天每道新题次日必闭卷重默，自动入队、次日默完即出队；一次默对+复杂度报对=焊住，不进错题本、不占 1/3/7/15；卡壳/写错/报错=转错题本第 1 档；走纯默写协议、约 6min/道快默、默完报复杂度；队列独立，不计库存水位/止血日；八股、项目不引入） | `rules/复习滚动机制.md` 第八节＋`rules/算法规范.md`「次日保温默」 | SKILL.md 复习节奏、计划规范第六步自检 3、复习模板（开场/过程/战报）、计划模板今日表/调整规则、示例场景二/十 |
| 算法必背口径（🔴 只认信号/模板骨架/复杂度，思路框架只用于理解不背诵） | `rules/算法规范.md` 记忆优先级表与风格第 2 节 | 算法模板必背清单、README 算法条 |
| 算法纯默写协议（复习段与保温默只默写不问框架、函数签名→关键行两档提示、错行补丁当天再默、默完报复杂度；到期复习按 2/1/0 调度，保温默按二元流向——一次默对=焊住 / 卡壳写错报错=转错题本，看题解是正常学法） | `rules/算法规范.md`「纯默写协议」节 | 复习滚动机制第一节入库表/第二节评级表/第四节算法钉子/第八节保温队列、示例场景十 |
| 排每日任务四自检（时长对目标 ±30min 必补/必砍、精力曲线排序、算法先到期默写再昨日保温默后 3 新题且任务行写清三个数字、项目块 ≤2 可观测点） | `rules/计划规范.md` 第六步 | SKILL.md 进度记录、计划模板今日表/明日表/调整规则、任务先行规范第二节、示例场景十 |
