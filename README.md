...
- 定计划：输入面试倒计时、每日可学时长、八股/算法/项目三围基础，输出三阶段总览 + 精确到块的今日任务表 + 里程碑；每天打卡按完成率动态调整（<50% 自动减半、欠债不滚利、输出跨天状态块），并在打卡后收集实际耗时，差异超 25% 时按题型系数校准后续排期（可关闭、可手动否决），有笔试和没项目/项目水两种边界都有专门排法
...
| `skill/learning-coach/references/templates/` | 六类输出模板：八股（含 🔴 必背清单与补丁式校正表）、算法（含必背清单）、项目（含级别函数地图和 L1 清单）、学习计划（含跨天状态块、减半日表、耗时反馈块与准确性报告）、模拟面试（开场/评分表/复盘）、错题本复习（入库/每日抽背/战报表）的完整输出格式，决定"长什么样" |
